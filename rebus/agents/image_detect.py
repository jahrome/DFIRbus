    ...
    def process(self, descriptor, sender_id):
        ...
        for part in volume:
            ...
            command = ('sudo losetup -P -f --show --offset %d --sizelimit %d %s' %
                       (part.start*volume.info.block_size,
                        part.len*volume.info.block_size,
                        case['hdd_location']))
            ...
            if 'Primary Table' in part.desc:
                ...
                if 'FVE-FS' in file(case['device'], 'rb').read(10):
                    ...
                    enc_key = file(os.path.join(case['casedir'], 'hdd', 'key.txt'),
                                   'r').read().strip()
                    command = 'bdemount -X allow_other -r %s %s %s' % (enc_key, case['device'],
                                                                       bdemntdir)
                    proc = subprocess.Popen(command, stdout=subprocess.PIPE,
                                            stderr=subprocess.PIPE, shell=True)
                    out, err = proc.communicate()
                    command = ('cp %s %s; umount %s; rmdir %s' %
                               (os.path.join(bdemntdir, 'bde1'),
                                os.path.join(case['casedir'], 'hdd'), bdemntdir, bdemntdir))
                    proc = subprocess.Popen(command, stdout=subprocess.PIPE,
                                            stderr=subprocess.PIPE, shell=True)
                    out, err = proc.communicate()
